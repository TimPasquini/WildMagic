...
def test_journal_lists_heard_rumor_with_hint_and_placeless_threat() -> None:
    ...
    assert storm_index == len(lines) - 1 or not lines[
        storm_index + 1
    ].strip().startswith("~")
...
def test_journal_empty_message() -> None:
    session = GameSession(seed=7, scenario="frontier", provider_name="mock")
    assert describe_journal(session.engine) == [
        "Your journal is empty. The world talks - listen to people."
    ]
...
def test_prophecy_spell_binds_incurs_debt_and_realizes(tmp_path: Path) -> None:
    ...
    try:
        ...
        session.execute_command(
            "cast somewhere north a blade waits for me",
            replay_wild_magic={
                "provider": "test",
        ...
        assert any(
            t.get("promise_id") == "promise_wild_debt" for t in state.event_timers
        )
        ...
        blades = [
            ...
            if e.kind == "item" and e.name.lower() == "named blade"
        ]
...
def test_debt_promise_settles_when_collector_arrives() -> None:
    ...
    session.execute_command(
        "cast borrow strength from tomorrow",
        replay_wild_magic={
            "provider": "test",
            "technical_failure": False,
            "error": None,
